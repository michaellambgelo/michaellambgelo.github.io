...
The fact also remains that the trivia questions are relatively secret and the leaderboard should not be — so the admin scoring app sits behind Cloudflare Access while the leaderboard stays publicly accessible on the same Cloudflare Pages instance. Cloudflare Access is great for free tier accounts that only need a handful of seats on the admin side.

The trivia games are separate and secret — only I know where they truly live.
