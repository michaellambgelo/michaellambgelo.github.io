...
Sometime in 2023 or 2024, two of my six Pis (`node5` and `node6`) quietly drifted off the LAN. I noticed in the way you notice a flickering light bulb — there, then gone, never the right priority to chase. uptime-kuma had been deployed to `node6` and would auto-start on reboot from a 2021-era backup. It still had outbound internet, so when monitors flapped it kept hitting Discord webhooks. The page itself returned a tunnel error to anyone who clicked the link, but I rarely opened it, and the Discord pings made it feel alive enough.
...
- **Nightly backups** of the kuma volume — container stopped briefly, tarball streamed out, container restarted, retained 31 days locally, and rsynced off-host to `node4` for a second copy. (Building this turned out to be its own [adventure with snap-confined Docker]({{ site.url }}/2026/04/snap-docker-bind-mount-backup.html); the obvious recipe from a [2023 post]({{ site.url }}/2023/12/container-relocation.html) doesn't work under Ubuntu's default Docker package, and the workaround is worth its own post.)
- **A push monitor** for `discord-embed-builder-presence` — the one Discord bot in the cluster I actually want to know about. (Setting this up, in turn, surfaced a [silent cron failure on snap-installed binaries]({{ site.url }}/2026/04/cron-snap-path-gotcha.html). A one-line cron job that took an hour to land.)
...
There's also a small physical-world consequence of this rebuild: the unlabeled Pis in the rack are no longer a problem. Each one now blinks its green ACT LED its own hostname digit, perpetually, courtesy of a tiny [systemd LED-ID service]({{ site.url }}/2026/04/cluster-led-id-systemd.html). Before today I literally couldn't tell `node5` from `node6`. Now I can identify either in five seconds from across the room.
...
[status.michaellamb.dev](https://status.michaellamb.dev) is live again. The monitor list is intentionally sparse — only the pieces I'm planning to actually pay attention to. Notifications go to Discord. If you click through and something looks broken, you'll probably see it before I do.
